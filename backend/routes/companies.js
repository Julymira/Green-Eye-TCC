...
// POST: Solicitar redefinição de senha (empresa)
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        await db.query(
            "DELETE FROM password_reset_tokens WHERE user_id = $1 AND user_type = 'company'",
            [company.id]
        );

        await db.query(
            "INSERT INTO password_reset_tokens (user_id, user_type, token, expires_at) VALUES ($1, 'company', $2, $3)",
...
// POST: Redefinir senha com token (empresa)
router.post('/reset-password', async (req, res) => {
    ...
    try {
        const result = await db.query(
            "SELECT * FROM password_reset_tokens WHERE token = $1 AND user_type = 'company' AND expires_at > NOW()",
            [token]
        );
        ...
        console.error("Erro ao redefinir senha (empresa):", error);
        res.status(500).json({ error: "Erro interno do servidor." });
    }
});
